...
def scatter(
    ...
    assert str(type(shap_values)).endswith(
        "Explanation'>"
    ), "The shap_values paramemter must be a shap.Explanation object!"

    # see if we are plotting multiple columns
    if (
        not isinstance(shap_values.feature_names, str)
        and len(shap_values.feature_names) > 0
    ):
        ...
        pl.subplots(1, len(inds), figsize=(min(6 * len(inds), 15), 5))
        for i in inds:
            ax = pl.subplot(1, len(inds), i + 1)
            scatter(
                shap_values[:, i], color=color, show=False, ax=ax, ymin=ymin, ymax=ymax
            )
    ...
    if str(type(color)).endswith("Explanation'>"):
        ...
        if issubclass(type(shap_values2.feature_names), (str, int)):
            ...
            shap_values_arr = np.hstack(
                [
                    shap_values_arr,
                    shap_values2.values.reshape(-1, len(feature_names) - 1),
                ]
            )
            features = np.hstack(
                [features, shap_values2.data.reshape(-1, len(feature_names) - 1)]
            )
            if shap_values2.display_data is None:
                display_features = np.hstack(
                    [
                        display_features,
                        shap_values2.data.reshape(-1, len(feature_names) - 1),
                    ]
                )
            else:
                display_features = np.hstack(
                    [
                        display_features,
                        shap_values2.display_data.reshape(-1, len(feature_names) - 1),
                    ]
                )
        else:
            ...
            if shap_values2.display_data is None:
                display_features = np.hstack(
                    [display_features, shap_values2.data[:, mask]]
                )
            else:
                display_features = np.hstack(
                    [display_features, shap_values2.display_data[:, mask]]
                )
        color = None
    ...
    if not hasattr(ind, "__len__"):
        if interaction_index == "auto":
            interaction_index = approximate_interactions(
                ind, shap_values_arr, features
            )[0]
        interaction_index = convert_name(
            interaction_index, shap_values_arr, feature_names
        )
    ...
    if len(shap_values_arr.shape) == 3 and hasattr(ind, "__len__") and len(ind) == 2:
        ...
        if ind1 == ind2:
            ...
            ax.set_ylabel(
                ...
                % (feature_names[ind1], feature_names[ind2])
            )
    ...
    if x_jitter > 0:
        ...
        if len(xvals) >= 2:
            ...
            xv += (np.random.random_sample(size=len(xv)) * jitter_amount) - (
                jitter_amount / 2
            )
    ...
    if hist:
        ...
        if (
            len(xvals) / len(xv_no_jitter) < 0.2
            and len(xvals) < 75
            and np.max(xvals) < 75
            and np.min(xvals) >= 0
        ):
